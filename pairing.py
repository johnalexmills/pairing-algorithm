...
def _box(content_lines, header=None, footer=None):
    """Wrap content lines in a double-line box (36-char interior)."""
    W = 36
    sep = "╠" + "═" * W + "╣"
    out = ["╔" + "═" * W + "╗"]
    if header:
...
def visualize_round(rnd):
    ...
    rnd: dict from next_round() / generate_night().
    """
    teams = rnd.get("teams", [])
    ...
    for tn, t1, t2 in tables:
        if t1 and t2:
            ...
            lines.append(f"Table {tn}")
            lines.append(f"  {a1} & {a2}")
            lines.append(f"  (awaiting opponent)")
            lines.append("")
        else:
            lines.append(f"Table {tn}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Crokinole pairing demo"
    )
    parser.add_argument(
        "--roster",
        default="Alice,Bob,Carol,Dave,Eve,Frank,Grace,Henry,Ivan,Jack",
        help="Comma-separated roster",
    )
    parser.add_argument(
        "--present",
        default=None,
        help="Comma-separated present players (default: all)",
    )
    parser.add_argument(
        "--tables", type=int, default=None,
        help="Number of tables (default: present // 4)",
    )
    parser.add_argument(
        "--rounds", type=int, default=3,
        help="Number of rounds to generate (default: 3)",
    )
    parser.add_argument(
        "--save", type=str, default=None,
        help="Save state to file for persistence demo",
    )
    args = parser.parse_args()

    roster = [s.strip() for s in args.roster.split(",")]
    present = (
        [s.strip() for s in args.present.split(",")]
        if args.present else roster
    )

    mgr = LeaguePairingManager(roster, args.save)
    rounds = mgr.generate_night(present, args.rounds, args.tables)

    print(f"\nRoster: {len(roster)} players")
    print(f"Present: {len(present)} players")
    print(f"Tables: {args.tables or max(1, len(present)//4)}")
    print()
